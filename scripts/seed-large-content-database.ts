...
const requireContentDatabaseUrl = () => {
  ...
  if (!databaseUrl) {
    throw new Error("Set BASEBUDDY_CONTENT_DATABASE_URL before running the large mapped-content seed.");
  }
...
const registerLoadTestProject = async (options: SeedOptions) => {
  if (!options.ownerEmail) {
    throw new Error("Pass --owner-email or set BASEBUDDY_LOAD_TEST_OWNER_EMAIL before --register-project.");
  }

  const owner = await findConfigUserByEmail(options.ownerEmail);

  if (!owner) {
    throw new Error(`No BaseBuddy config user found for ${options.ownerEmail}. Create the user, then rerun registration.`);
  }
  ...
  const project = existingProject.project
    ? await updateConfigProjectMetadata({
        ...
        websiteUrl: `https://load-test.local/${options.schema}`,
      })
...
const main = async () => {
  ...
  try {
    ...
    const registeredProject = options.registerProject
      ? await registerLoadTestProject(options)
      : null;
